...
st.write(
    "This is an example web scraper for Airbnb made using Streamlit. Please note that this is not a complete model, but rather a demonstration of how web scraping can be done with Streamlit."
)

st.image("./images/airbnb_scrapper_logo.jpg", caption=None, use_column_width=True)
...
with countcol4:
    pets_count = st.number_input("Number of pets", value=0, key="pets_count", step=1)
...
if additional_search_filters:
    stay_radio = st.radio("Stay for a:", ["Weekend", "Week", "Month"], horizontal=True)
    ...
    with pricecol1:
        min_price = st.number_input("Minimum Price", value=10, key="min_price", step=1)
        st.write(f"Minimum Price {min_price} USD")
    with pricecol2:
        max_price = st.number_input("Maximum Price", value=450, key="max_price", step=1)
...
st.markdown(
    "I specialize in 3D Model design and development, Machine Learning, Artificial Intelligence, and Chatbot Development. With extensive experience in academic research and practical applications, I provide comprehensive solutions from data preprocessing to deployment."
)

st.markdown("**My expertise encompasses:**")
st.markdown(
    "- Integration and optimization of GPT Models for diverse applications including API Integration, LangChain, and LLamaIndex."
)
st.markdown("- Expertise in Data Preprocessing and Visualization techniques.")
st.markdown(
    "- JavaScript development for dynamic web applications, utilizing frameworks such as Three.js for immersive 3D visualizations."
)
...
st.markdown(
    "My recent projects have involved the development of a web scrapping tool, extracting complex location data from Airbnb using the Selenium library, the presentation of the project is based on the Python Streamlit library."
)
