...
location_radio = st.radio("Search Type:", ["Search by location", "Search by region"], horizontal=True)
...
if location_radio == "Search by location":
    ...
    location = st.selectbox("Select Region", ["I'm flexible","Middle East","United Arab Emirates","Europe","United Kingdom","Southeast Asia"], index=0)
...
with countcol1:
    adults_count = st.number_input("Number of adults", value = 0, key="adults_count", step = 1)
with countcol2:
    children_count = st.number_input("Number of children", value = 0, key="children_count", step = 1)
with countcol3:
    infants_count = st.number_input("Number of infants", value = 0, key="infant_count", step = 1)
with countcol4:
...
st.markdown("- Programming languages like Python for backend development and JavaScript for front end.")
st.markdown("- Utilization of CSV and Google Sheets for data management.")
st.markdown("- Data manipulation libraries such as Pandas and NumPy and visualization tools like Matplotlib.")
st.markdown("- Proficient in version control with Git and leveraging Github actions for seamless workflows.")
...
st.markdown("LinkedIn: [Ashhad Ahmed](https://www.linkedin.com/in/ashhad-ahmed-1230ab25a/)")
st.markdown("Stack Overflow: [AshhadDevLab](stackoverflow.com/users/23106915/ashhaddevlab)")